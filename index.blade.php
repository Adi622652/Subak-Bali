...
<section class="bg-white shadow-sm sticky top-20 z-30">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
        <form action="{{ route('subak.index') }}" method="GET" class="flex flex-col md:flex-row md:items-center gap-4">
            <!-- Kabupaten Filter -->
            <div class="md:w-48">
            ...
            <div class="flex-1 relative">
                <input type="text" name="search" value="{{ $search }}" placeholder="Cari nama subak..." class="w-full border border-green-pale rounded-lg px-4 py-2 pr-10 focus:outline-none focus:border-green-medium focus:ring-1 focus:ring-green-medium">
            ...
            <div class="text-gray-600 text-sm whitespace-nowrap">
                Menampilkan {{ $subaks->total() }} subak
            </div>
        </form>
    </div>
</section>
        ...
        @if($subaks->count() > 0)
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                ...
                <div class="group relative aspect-square rounded-2xl overflow-hidden cursor-pointer">
                    ...
                    @if($subak->foto_url)
                        <img src="{{ $subak->foto_url }}" alt="{{ $subak->nama }}" class="w-full h-full object-cover transition-transform duration-300 group-hover:scale-110">
                    @else
                    ...
                    <div class="absolute top-4 left-4">
                        <span class="bg-green-dark/90 text-white text-xs font-medium px-3 py-1 rounded-full">{{ $subak->kabupaten }}</span>
                    </div>
                    
                    <!-- Default Content (Bottom) -->
                    <div class="absolute bottom-4 left-4 right-4">
                        <h3 class="text-white font-bold text-lg">{{ $subak->nama }}</h3>
                    </div>
                    ...
                    <div class="absolute inset-0 bg-green-dark/85 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
                        <div class="text-center text-white p-4 transform translate-y-4 group-hover:translate-y-0 transition-transform duration-300">
                            <h3 class="text-xl font-bold mb-1">{{ $subak->nama }}</h3>
                            <p class="text-white/80 text-sm mb-1">{{ $subak->kecamatan }}, {{ $subak->kabupaten }}</p>
                            <p class="text-white/80 text-sm mb-1">{{ $subak->luas_subak }}</p>
                            <p class="text-white/80 text-sm mb-4">{{ $subak->jumlah_anggota }}</p>
                            <a href="{{ route('subak.show', $subak->slug) }}" class="inline-block border-2 border-white text-white px-6 py-2 rounded-full font-medium hover:bg-white hover:text-green-dark transition-colors">
                                Lihat Detail &rarr;
                            </a>
                        </div>
                    </div>
