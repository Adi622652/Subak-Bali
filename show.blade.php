...
@section('title', $subak->nama . ' - Detail Subak')
...
<section class="relative min-h-[50vh] flex items-center justify-center overflow-hidden">
    ...
    <div class="absolute inset-0">
        @if($subak->foto_url)
            <img src="{{ $subak->foto_url }}" alt="{{ $subak->nama }}" class="w-full h-full object-cover scale-105 transition-transform duration-1000 group-hover:scale-100">
        @else
    ...
    <div class="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full text-center mt-20">
        ...
        <div class="inline-block p-8 mb-8 animate-fade-in-up">
            <span class="inline-block bg-green-pale text-green-dark text-[10px] font-bold uppercase tracking-[0.2em] px-4 py-1.5 rounded-full mb-4 shadow-sm">
                <i class="fa-solid fa-location-dot mr-1.5"></i> {{ $subak->kabupaten }}
            </span>
            <h1 class="text-4xl md:text-6xl font-bold text-white mb-6 drop-shadow-lg leading-tight">{{ $subak->nama }}</h1>
            ...
            <nav class="flex items-center justify-center space-x-2 text-white/70 text-xs font-semibold uppercase tracking-widest">
                ...
                <a href="{{ route('subak.index') }}" class="hover:text-white transition-colors">Data Subak</a>
                <span class="opacity-30">/</span>
                <span class="text-white">{{ $subak->nama }}</span>
            </nav>
        </div>
    </div>
</section>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-12">
            ...
            <div class="lg:col-span-8 space-y-10" x-data="{ activeTab: 'umum' }">
                        ...
                        <div class="bg-white rounded-[2rem] p-8 shadow-sm">
                            ...
                            <div class="grid grid-cols-2 lg:grid-cols-4 gap-6">
                                <div class="p-4 bg-gray-50 rounded-2xl">
                                    ...
                                    <p class="text-sm font-semibold text-green-dark">{{ $subak->batas_utara ?? '-' }}</p>
                                </div>
                                <div class="p-4 bg-gray-50 rounded-2xl">
                                    <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Selatan</p>
                                    <p class="text-sm font-semibold text-green-dark">{{ $subak->batas_selatan ?? '-' }}</p>
                                </div>
                                <div class="p-4 bg-gray-50 rounded-2xl">
                                    <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Timur</p>
                                    <p class="text-sm font-semibold text-green-dark">{{ $subak->batas_timur ?? '-' }}</p>
                                </div>
                                <div class="p-4 bg-gray-50 rounded-2xl">
                                    <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Barat</p>
                                    <p class="text-sm font-semibold text-green-dark">{{ $subak->batas_barat ?? '-' }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div x-show="activeTab === 'irigasi'" x-transition:enter="transition ease-out duration-300 transform" x-transition:enter-start="opacity-0 translate-y-4" x-transition:enter-end="opacity-100 translate-y-0" class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="bg-white p-6 rounded-3xl shadow-sm hover:shadow-md transition-shadow">
                            <div class="flex items-center space-x-4">
                                ...
                                <div>
                                    <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-0.5">{{ $item['label'] }}</p>
                                    @if(isset($item['isBadge']))
                                        ...
                                        @endphp
                                        <span class="inline-block px-3 py-1 rounded-full text-[10px] font-bold uppercase {{ $badgeClass }}">{{ $item['value'] }}</span>
                                    @else
                                        <p class="text-sm font-bold text-green-dark">{{ $item['value'] }}</p>
                                    @endif
                                </div>
                            </div>
                        </div>
                        ...
                        @if($subak->foto_url)
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div class="group relative aspect-video rounded-[2rem] overflow-hidden shadow-xl">
                                    <img src="{{ $subak->foto_url }}" alt="{{ $subak->nama }}" class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110">
                ...
                <div class="flex items-center justify-between border-t border-gray-200 pt-10">
                    @if($prevSubak)
                        <a href="{{ route('subak.show', $prevSubak->slug) }}" class="flex items-center group">
                            ...
                            <div class="ml-4 text-left hidden sm:block">
                                ...
                                <p class="text-sm font-bold text-green-dark">{{ $prevSubak->nama }}</p>
                            </div>
                        </a>
                    @else
                        <div></div>
                    ...
                    <a href="{{ route('subak.index') }}" class="px-8 py-3 bg-white rounded-full text-xs font-bold uppercase tracking-widest text-green-medium hover:bg-green-medium hover:text-white transition-all shadow-sm">
                        Data Subak
                    </a>

                    @if($nextSubak)
                        <a href="{{ route('subak.show', $nextSubak->slug) }}" class="flex items-center group">
                            <div class="mr-4 text-right hidden sm:block">
                                ...
                                <p class="text-sm font-bold text-green-dark">{{ $nextSubak->nama }}</p>
                            </div>
                            <div class="w-10 h-10 rounded-full border border-gray-200 flex items-center justify-center group-hover:bg-green-medium group-hover:text-white group-hover:border-green-medium transition-all">
            ...
            <div class="lg:col-span-4 space-y-8">
                ...
                <div class="bg-green-dark rounded-[2.5rem] p-8 text-white shadow-2xl shadow-green-900/40 relative overflow-hidden group">
                    ...
                    <div class="space-y-6 relative z-10">
                        <div class="flex items-center justify-between border-b border-white/10 pb-4">
                            ...
                            <span class="text-sm font-bold">{{ $subak->desa }}</span>
                        </div>
                        <div class="flex items-center justify-between border-b border-white/10 pb-4">
                            <div class="flex items-center">
                            ...
                            <span class="text-sm font-bold">{{ $subak->kecamatan }}</span>
                        </div>
                        <div class="flex items-center justify-between border-b border-white/10 pb-4">
                            <div class="flex items-center">
                            ...
                            <span class="text-sm font-bold">{{ $subak->luas_subak }}</span>
                        </div>
                        <div class="flex items-center justify-between">
                            ...
                            <span class="text-sm font-bold">{{ $subak->jumlah_anggota }} Orang</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-[2.5rem] p-8 shadow-sm text-center">
                    ...
                    <h4 class="text-xl font-bold text-green-dark mb-1">{{ $subak->nama_pekaseh ?? 'Nama Tidak Ada' }}</h4>
                    <p class="text-gray-500 text-sm mb-8">Pimpinan Organisasi Subak</p>
                    
                    <a href="https://wa.me/{{ preg_replace('/[^0-9]/', '', $subak->no_telp) }}" target="_blank" class="block w-full bg-green-bg text-green-dark py-4 rounded-2xl font-bold text-sm hover:bg-green-medium hover:text-white transition-all shadow-sm">
